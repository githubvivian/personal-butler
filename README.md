...
- 日历：日/周视图、本地提醒
- 悬而未决：任务/报账/评审跟踪、延期提醒
- 家庭：贝贝 / 核桃课表与活动（多选筛选）
- 生日：阴历 / 阳历、倒计时
- 灵感库、密码保险库（无银行卡）
- 加密数据库记录备份与同安装环境恢复（.pbak；不是完整换机备份）
- 指纹解锁，2 小时内免重复验证
...
> [!WARNING]
> 当前 CI 生成的是 **release 模式、debug 密钥签名**的 APK，仅适合测试和个人使用。不同电脑或 CI 环境生成的 APK 可能使用不同 debug 密钥，不能保证可以覆盖安装。若要长期保留数据并持续升级，应创建、妥善备份并始终使用同一个 release keystore。当前 `.pbak` 只保存数据库记录，不包含相册原图；密码保险库还依赖未写入备份的本机安全密钥，因此不能作为卸载、清除应用数据或换机后的完整灾难恢复方案。在正式签名和可迁移备份方案完成前，不要依赖 `.pbak` 后卸载含重要数据的现有安装。
...
4. 可在「我的 → 数据备份与恢复」导出加密数据库记录备份；先阅读页面限制，不要把它当作卸载或换机的完整备份
...
- 密码库字段单独 AES 加密
- 当前 `.pbak` 不包含相册原图，也不包含解密密码保险库所需的本机安全密钥；同一安装环境内可用于数据库记录恢复，但卸载、清除数据或换机后不保证保险库可解密
- 默认不上传云端
